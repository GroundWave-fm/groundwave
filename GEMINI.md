...
5. **No Secret Leaks**: Never hardcode API keys, Stripe secrets, or credentials. Always update `.env.example` when introducing new environment variables.
...
* **Examples**:
  ...
  * `docs(master-plan): add Casual Listener persona and Scene Radio specs`
...
### 3.1 Pre-PR Checklist (The Quality Gate)
...
## 4. Agent Guidelines & Collaboration Rules
...
2. **Execute Locally & Verify**: Never claim a task is complete without running `pnpm build` or testing the execution output.
